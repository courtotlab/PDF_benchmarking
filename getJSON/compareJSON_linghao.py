...
def compare_gliner_output(template, data, hospital, model_name, json_file):
    """
...
def extract_hospital_from_template(expected_report):
    ...
    Converts all keys and values to lowercase for consistent comparison.
    """
    
    #lab mapping to dir names
    lab_to_hospital = {
    ...
    if type(expected_report) == str:
        import json
        expected_report = json.loads(expected_report)

    #get the hospital from the mock data
    if "testing_laboratory" in expected_report:
        lab_name = expected_report.get("testing_laboratory", "").lower()
        hospital = lab_to_hospital.get(lab_name)
    else:
...
def linghao_code(data_dir, mock_data_test_output):
    ...
    #iterate through each report id
    for keys in mock_report_outputs.keys():

        #pdf_path = mock_data_dir+f"report_{keys}.pdf"
        expected_report, hospital = extract_hospital_from_template(mock_report_outputs[keys]["expected_report"])
        ...
        if differences:
            field_analysis = analyze_fp_fn_by_field(differences, hospital, model_name, expected_report, parsed_output_json)
